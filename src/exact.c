...
extern int  do_round(char *Solver, char *InFileName, double base, double *UpperBound, double *LowerBound, long auditing, 
			char *SolutionFile, char* StatisticsFile, char *LicenseFile, char *LogFile,
			long MaxTime, long ZeroRestricted, char *NamePathExe, 
			double *MaxJump, long *NumberJump, double *UsedTime,
            long *ErrorCode);
...
int main(int argc, char *argv[])
{
    ...
    long MaxTime, NumberJump, ErrorCode, ZeroRestricted, audit;
    ...
    audit = atol(argv[3]);                    // option: 0 means no rounding post-optimality phase is required
    ...
    printf("File=%s Rounder=%s  base=%.1f  audit=%ld\n",InFileName,Solver,Base,audit);
